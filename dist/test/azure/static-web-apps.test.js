...
describe("deploySwaZip", () => {
    ...
    it("gets deployment token and POSTs ZIP buffer", async () => {
        ...
        mockFetch((url, init) => {
            if (url.includes("/staticSites/my-app") &&
        ...
        assert.ok(zipCall.url.includes("my-app.azurestaticapps.net"), "URL should contain the SWA hostname");
        // Verify Authorization header is raw token (not Bearer)
        const headers = zipCall.init?.headers;
