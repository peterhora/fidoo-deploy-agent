...
function mockTokensWithUpn(upn) {
...
async function setupAppDir(files = { "index.html": "<h1>Hello</h1>" }) {
 ...
 */
function mockBlobAndDeployFlow(slug, registryStatus = 404) {
    ...
    // blob delete (cleanup _deploy-temp)
    mockFetch((url, init) => {
...
describe("app_deploy — first deploy", () => {
    ...
    it("uploads files to blob, updates registry, deploys site, writes .deploy.json", async () => {
        ...
        const blobPuts = calls.filter((c) => c.url.includes(".blob.core.windows.net") && c.init?.method === "PUT");
        ...
        const zipdeployCall = calls.find((c) => c.url.includes("zipdeploy") && c.init?.method === "POST");
    ...
    it("returns URL with path-based pattern", async () => {
        ...
        assert.ok(body.url.includes("ai-apps.env.fidoo.cloud/my-app/"), `URL should be path-based, got: ${body.url}`);
    });
    it("includes deployedBy from JWT UPN", async () => {
        // Re-setup tokens with a UPN-bearing JWT
        await rm(join(tokenDir, "tokens.json"), { force: true });
        await writeFile(join(tokenDir, "tokens.json"), JSON.stringify(mockTokensWithUpn("alice@fidoo.cloud")));
        mockBlobAndDeployFlow("my-app", 404);
        const result = await handler({
            folder: appDir,
            app_name: "My App",
        ...
        const calls = getFetchCalls();
        const registryPut = calls.find((c) => c.url.includes(".blob.core.windows.net") &&
        ...
        const registryJson = body instanceof Uint8Array
            ? Buffer.from(body).toString("utf-8")
            : typeof body === "string"
                ? body
                : String(body);
        const registry = JSON.parse(registryJson);
        const entry = registry.apps.find((a) => a.slug === "my-app");
...
describe("app_deploy — re-deploy", () => {
    ...
    it("returns URL with path-based pattern", async () => {
        ...
        assert.ok(!result.isError, `Expected success but got: ${result.content[0].text}`);
        const body = JSON.parse(result.content[0].text);
        assert.ok(body.url.includes("ai-apps.env.fidoo.cloud/existing-app/"), `URL should be path-based, got: ${body.url}`);
    });
...
//# sourceMappingURL=app-deploy.test.js.map