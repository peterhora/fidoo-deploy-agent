 ...
 */
export function extractDisplayName(token: string): string | undefined {
  try {
    const parts = token.split(".");
    if (parts.length !== 3) return undefined;
    ...
    const upnLike: string | undefined = payload.preferred_username || payload.upn;
    if (!upnLike) return undefined;
