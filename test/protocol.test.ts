...
describe("parseJsonRpcMessage", () => {
  it("parses a valid request", () => {
    ...
    assert.equal((msg as JsonRpcRequest).id, 1);
  ...
  it("parses a request with string id", () => {
    ...
    assert.equal((msg as JsonRpcRequest).id, "abc");
  });
  ...
  it("rejects missing jsonrpc field", () => {
    assert.throws(
      () => parseJsonRpcMessage('{"id":1,"method":"test"}'),
      /jsonrpc/
    );
  });
  ...
  it("rejects wrong jsonrpc version", () => {
    assert.throws(
      () => parseJsonRpcMessage('{"jsonrpc":"1.0","id":1,"method":"test"}'),
      /jsonrpc/
    );
  });
  ...
  it("rejects missing method", () => {
    assert.throws(
      () => parseJsonRpcMessage('{"jsonrpc":"2.0","id":1}'),
      /method/
    );
  });
